...
fn inject_nav_script(html: &[u8]) -> Vec<u8> {
    ...
    if let Some(pos) = body.find("</head>") {
        ...
        let mut result = Vec::with_capacity(html.len() + NAV_SCRIPT.len());
        result.extend_from_slice(&html[..byte_pos]);
        result.extend_from_slice(NAV_SCRIPT.as_bytes());
        result.extend_from_slice(&html[byte_pos..]);
        return result;
    }
